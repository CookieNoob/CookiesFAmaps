...
local Tables = import('/maps/adaptive_setons_clutch.v0002/adaptive_setons_clutch_tables.lua')
...
-- get scenario info about how the map should look like, if no option is changed, change it to the default
    ...
    local optional_reclaim_middle = ScenarioInfo.Options.optional_reclaim_middle or 4
    ...
    LOG("ULTIMATE", automex, hydroplus, optional_reclaim_middle, optional_reclaim_back, middlemex, sidemex, underwatermex, optional_reclaim, mirrormex, optional_civilian_base)
    -- get map markers
...
------------------------------------------------------------------------
    ...
    if(optional_reclaim_back > 1) then
        AddFactionReclaimBack(optional_reclaim_back)
    end
