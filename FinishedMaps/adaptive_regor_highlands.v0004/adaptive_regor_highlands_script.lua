...
local Tables = import('/maps/adaptive_regor_highlands.v0004/adaptive_regor_highlands_tables.lua')
...
local centermex = ScenarioInfo.Options.centermex or 2
local sidemex = ScenarioInfo.Options.sidemex or 3
local underwatermex = ScenarioInfo.Options.underwatermex or 5    
local optional_reclaim = ScenarioInfo.Options.optional_reclaim or 4
...
local backmex = ScenarioInfo.Options.backmex or 4
...
------------------------------------------------------------------------
    LOG(optional_reclaim, "optional_reclaim")
    if(optional_reclaim_side > 1 and optional_reclaim_side < 6) then
        ScenarioUtils.CreateArmyGroup('ARMY_17', 'Optional_Wreckage_Side_'.. optional_reclaim_side, true)
    end
    
    if (optional_reclaim > 1 and optional_reclaim < 8 and optional_reclaim != 4) then
        ScenarioUtils.CreateArmyGroup('ARMY_17', 'Optional_Wreckage_' .. optional_reclaim, true)
    end 
    if (optional_reclaim == 4 or optional_reclaim == 5) then
    ...
    if(optional_civilian_base > 1) then
        ScenarioUtils.CreateArmyGroup('ARMY_17', 'Optional_Civilian_2', false)
        local spawncivs = true
        for m = 11, 12 do
            armystring = "ARMY_" .. m
            for _, army in ListArmies() do
                if( army == armystring) then
                    LOG("ULTIMATE found player in civ base. remove civ defences")
                    spawncivs = false
                end
            end
        end
        if(spawncivs) then
            if(optional_civilian_base == 3) then
                ScenarioUtils.CreateArmyGroup('ARMY_17', 'Optional_Civilian_3', false)
            elseif (optional_civilian_base == 4) then
                ScenarioUtils.CreateArmyGroup('ARMY_17', 'Optional_Civilian_4', false)
            end
        end
    end
...
-- corresponds to player one, the second to player 2 and so on... load it from the _tables.lua file
    ...
    local spwnAdditionalcentermass = Tables.centermass or {}
...
-- loop over all markers of mexes in the table of the row of the missing army
            ...
            for l = 1, centermex - 1   do
                doit=FalseIfInList(name, spwnAdditionalcentermass[l], MassString, doit);
            end
