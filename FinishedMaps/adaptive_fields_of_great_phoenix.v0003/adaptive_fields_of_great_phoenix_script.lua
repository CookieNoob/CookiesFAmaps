...
local Tables = import('/maps/adaptive_fields_of_great_phoenix.v0003/adaptive_fields_of_great_phoenix_tables.lua')
...
local middlemex = ScenarioInfo.Options.middlemex or 3
local sidemex = ScenarioInfo.Options.sidemex or 3
local underwatermex = ScenarioInfo.Options.underwatermex or 4
local backmex = ScenarioInfo.Options.backmex or 1
local islandmex = ScenarioInfo.Options.islandmex or 1
local optional_reclaim = ScenarioInfo.Options.optional_reclaim or 2
local mirrormex = ScenarioInfo.Options.mirrormex or 1
local optional_civilian_base_side = ScenarioInfo.Options.optional_civilian_base_side or 1
local optional_civilian_base_middle = ScenarioInfo.Options.optional_civilian_base_middle or 1
...
-- Script by CookieNoob and KeyBlue
    ...
    LOG("ULTIMATE", automex, hydroplus, optional_reclaim_middle, optional_reclaim_adaptive, middlemex, sidemex, underwatermex, optional_reclaim, mirrormex)
    -- get map markers
...
------------------------------------------------------------------------
    ...
    if ((optional_reclaim == 2) or (optional_reclaim == 5) or (optional_reclaim == 7) ) then
        ScenarioUtils.CreateArmyGroup('ARMY_17', 'Optional_Wreckage_2', true)
    end
    if ((optional_reclaim == 3) or (optional_reclaim == 4) or (optional_reclaim == 5) or (optional_reclaim == 7) ) then
        ScenarioUtils.CreateArmyGroup('ARMY_17', 'Optional_Wreckage_3', true)
    end
    if ((optional_reclaim == 4) or (optional_reclaim == 5) or (optional_reclaim == 7) ) then
        ScenarioUtils.CreateArmyGroup('ARMY_17', 'Optional_Wreckage_4', true)
    end
    if ((optional_reclaim == 6) or (optional_reclaim == 7) ) then
        ScenarioUtils.CreateArmyGroup('ARMY_17', 'Optional_Wreckage_5', true)
    end
    
    
    
    if(optional_civilian_base_side > 1) then
        ScenarioUtils.CreateArmyGroup('ARMY_17', 'Optional_Civilians_Side_Base', false)
        if(optional_civilian_base_side == 3 or optional_civilian_base_side == 4) then
            ScenarioUtils.CreateArmyGroup('ARMY_17', 'Optional_Civilians_Side_Defences_2', false)
        elseif(optional_civilian_base_side == 5 or optional_civilian_base_side == 6) then
            ScenarioUtils.CreateArmyGroup('ARMY_17', 'Optional_Civilians_Side_Defences_3', false)
        end
        if(optional_civilian_base_side == 4 or optional_civilian_base_side == 6) then
            ScenarioUtils.CreateArmyGroup('ARMY_17', 'Optional_Civilians_Side_Shields', false)
        end
    end

    if(optional_civilian_base_middle > 1) then
        ScenarioUtils.CreateArmyGroup('ARMY_17', 'Optional_Civilians_Middle_Wrecks', true)
        ScenarioUtils.CreateArmyGroup('ARMY_17', 'Optional_Civilians_Middle_Base', false)
        if(optional_civilian_base_middle == 3) then
            ScenarioUtils.CreateArmyGroup('ARMY_17', 'Optional_Civilians_Middle_Defences', false)
        end
    end
...
-- corresponds to player one, the second to player 2 and so on... load it from the _tables.lua file
    ...
    local spwnAdditionalmass = Tables.additionalmex or {}
...
-- loop over all markers of mexes in the table of the row of the missing army
            ...
            if additionalMex == 1 then
                doit = FalseIfInList(name, spwnAdditionalmass, MassString, doit);
            end
        end
