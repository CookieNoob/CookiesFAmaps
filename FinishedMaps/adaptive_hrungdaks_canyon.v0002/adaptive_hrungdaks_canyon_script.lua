...
local Tables = import('/maps/adaptive_hrungdaks_canyon.v0002/adaptive_hrungdaks_canyon_tables.lua')
...
-- get scenario info about how the map should look like, if no option is changed, change it to the default
    ...
    local optional_reclaim_back = ScenarioInfo.Options.optional_reclaim_back or 1
    ...
    LOG("ULTIMATE", automex, hydroplus, optional_reclaim_middle, optional_reclaim_back, middlemex, sidemex, underwatermex, optional_reclaim, mirrormex, optional_civilian_base, jamming)
    -- get map markers
...
------------------------------------------------------------------------
    if(jamming < 4) then
        for i = 2, jamming do
            ScenarioUtils.CreateArmyGroup('ARMY_17', 'Jamming_' .. i, false)
        end
    end
    
    if(optional_reclaim > 1 and optional_reclaim < 5) then
        ScenarioUtils.CreateArmyGroup('ARMY_17', 'Optional_Reclaim_'.. optional_reclaim, true)
    end
...
--add civilian base
    if(optional_civilian_base > 1) then
        ...
        if( (optional_civilian_base - math.floor(optional_civilian_base/2) * 2) == 1) then
            ScenarioUtils.CreateArmyGroup('ARMY_17', 'Optional_Civilians_Shields', false)
        end
        if(optional_civilian_base == 4 or optional_civilian_base == 5) then
            ScenarioUtils.CreateArmyGroup('ARMY_17', 'Optional_Civilians_Defences_2', false)
        elseif (optional_civilian_base == 6 or optional_civilian_base == 7) then
            ScenarioUtils.CreateArmyGroup('ARMY_17', 'Optional_Civilians_Defences_3', false)
        elseif (optional_civilian_base == 8 or optional_civilian_base == 9) then
            ScenarioUtils.CreateArmyGroup('ARMY_17', 'Optional_Civilians_Defences_3', false)
            ScenarioUtils.CreateArmyGroup('ARMY_17', 'Optional_Civilians_Defences_4', false)
        end
        if(optional_civilian_base > 3) then
            ScenarioUtils.CreateArmyGroup('ARMY_17', 'Optional_Civilians_Walls_2', false)
        end
    end
