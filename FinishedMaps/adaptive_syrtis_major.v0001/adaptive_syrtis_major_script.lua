...
local Tables = import('/maps/adaptive_syrtis_major.v0001/adaptive_syrtis_major_tables.lua')
...
local optional_civilian_base = ScenarioInfo.Options.optional_civilian_base or 5
local dupicatesinglemex = ScenarioInfo.Options.dupicatesinglemex or 1
local removeRock = ScenarioInfo.Options.removeRock or 1
local platmex = ScenarioInfo.Options.platmex or 1
...
------------------------------------------------------------------------
    ...
    if(optional_civilian_base > 1) then
        ...
        ScenarioUtils.CreateArmyGroup('ARMY_17', 'Optional_Civilian_Walls', false)

        if(optional_civilian_base == 2 or optional_civilian_base == 5 or optional_civilian_base == 8) then
            ScenarioUtils.CreateArmyGroup('ARMY_17', 'Optional_Civilian_Defences_2', false)
        elseif(optional_civilian_base == 3 or optional_civilian_base == 6 or optional_civilian_base == 9) then
            ScenarioUtils.CreateArmyGroup('ARMY_17', 'Optional_Civilian_Defences_3', false)
        end
        
        if(optional_civilian_base > 4 and optional_civilian_base < 8) then
            ScenarioUtils.CreateArmyGroup('ARMY_17', 'Optional_Civilian_Base_2', false)
        elseif(optional_civilian_base > 7) then
            ScenarioUtils.CreateArmyGroup('ARMY_17', 'Optional_Civilian_Base_3', false)
        end
    end
...
-- corresponds to player one, the second to player 2 and so on... load it from the _tables.lua file
    ...
    local spwnAdditionalplatmass = Tables.platmex or {}
...
-- loop over all markers of mexes in the table of the row of the missing army
            ...
            for l = 1, platmex - 1  do
                doit=FalseIfInList(name, spwnAdditionalplatmass[l], MassString, doit);
            end
            if(automex == 6) then
