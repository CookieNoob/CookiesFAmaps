...
local Tables = import('/maps/adaptive_twin_rivers.v0002/adaptive_twin_rivers_tables.lua')
...
-- Script by CookieNoob and KeyBlue  
    ...
    local optional_reclaim_middle = ScenarioInfo.Options.optional_reclaim_middle or 2
    local optional_reclaim_side = ScenarioInfo.Options.optional_reclaim_side or 1
    local middlemex = ScenarioInfo.Options.middlemex or 4
    ...
    LOG("ULTIMATE", automex, hydroplus, optional_reclaim_middle, middlemex, sidemex, optional_reclaim_side, mirrormex, dupicatesinglemex)
    -- get map markers
    local markers = ScenarioUtils.GetMarkers();
    
    if(optional_reclaim_middle < 5) then 
        for i = 2, optional_reclaim_middle do
            ScenarioUtils.CreateArmyGroup('ARMY_17', 'Optional_Wreckage_Middle_' .. i, true)
        end
    end
    
    if ((optional_reclaim_side == 2) or (optional_reclaim_side == 3)) then
        ScenarioUtils.CreateArmyGroup('ARMY_17', 'Optional_Wreckage_Side_' .. optional_reclaim_side, true)
    elseif (optional_reclaim_side == 4) then
        ScenarioUtils.CreateArmyGroup('ARMY_17', 'Optional_Wreckage_Side_2', true)
        ScenarioUtils.CreateArmyGroup('ARMY_17', 'Optional_Wreckage_Side_3', true)
    end
...
-- corresponds to player one, the second to player 2 and so on... load it from the _tables.lua file
    ...
    local spwnAdditionalunderwatermass = Tables.underwatermass or {}
...
-- loop over all markers of mexes in the table of the row of the missing army
            ...
            for l = 1, underwatermex - 1  do
                doit=FalseIfInList(name, spwnAdditionalunderwatermass[l], MassString, doit);
            end
        end
