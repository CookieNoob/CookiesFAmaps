...
local Tables = import('/maps/adaptive_point_of_reason.v0002/adaptive_point_of_reason_tables.lua')
...
local sidemex1 = ScenarioInfo.Options.sidemex1 or 3 
local sidemex2 = ScenarioInfo.Options.sidemex2 or 3
...
------------------------------------------------------------------------
    ...
    elseif(optional_reclaim > 4 and optional_reclaim < 8) then
        for i = 2, optional_reclaim - 2 do
            ScenarioUtils.CreateArmyGroup('ARMY_17', 'Optional_Wreckage_'.. i, true)
        end
    end
    
    if optional_reclaim_middle < 6 then
        for i = 2, optional_reclaim_middle do
            ScenarioUtils.CreateArmyGroup('ARMY_17', 'Optional_Wreckage_Mid_'.. i , true)
        end
    end
    if(optional_civilian_base > 1) then
        ScenarioUtils.CreateArmyGroup('ARMY_17', 'Optional_Civilians_Base', false)
        ScenarioUtils.CreateArmyGroup('ARMY_17', 'Optional_Civilians_Walls', false)
        if(optional_civilian_base > 2 and optional_civilian_base < 6) then
            ScenarioUtils.CreateArmyGroup('ARMY_17', 'Optional_Civilians_Defences_'.. (optional_civilian_base - 1), false)
        end
    end
...
-- corresponds to player one, the second to player 2 and so on... load it from the _tables.lua file
    ...
    local spwnAdditionalsidemass_island = Tables.sidemass_island;
    local spwnAdditionalsidemass_underwater = Tables.sidemass_underwater;
    local crazyrushOneMex = Tables.crazyrushOneMex;
    local DuplicateListMex = Tables.DuplicateListMex;
...
-- loop over all markers of mexes in the table of the row of the missing army
            ...
            for l = 1, sidemex1 - 1  do
                doit=FalseIfInList(name, spwnAdditionalsidemass_island[l], MassString, doit);
            end
            for l = 1, sidemex2 - 1  do
                doit=FalseIfInList(name, spwnAdditionalsidemass_underwater[l], MassString, doit);
            end
        end
