...
local Tables = import('/maps/adaptive_argon.v0002/adaptive_argon_tables.lua')
...
local middlemex = ScenarioInfo.Options.middlemex or 2
local sidemex = ScenarioInfo.Options.sidemex or 2
...
local optional_civilian_base_side_shields = ScenarioInfo.Options.optional_civilian_base_side_shields or 1
local optional_civilian_base_middle_shields = ScenarioInfo.Options.optional_civilian_base_middle_shields or 1
...
------------------------------------------------------------------------
    ...
    if optional_reclaim < 6 then
        for i = 2, optional_reclaim do
            ScenarioUtils.CreateArmyGroup('ARMY_17', 'Optional_Wreckage_' .. i, true)
        end
    end
    ...
    if(optional_civilian_base_middle > 1) then
        ...
        if(optional_civilian_base_middle_shields > 1 and optional_civilian_base_middle_shields < 5) then
            ScenarioUtils.CreateArmyGroup('ARMY_17', 'Optional_Civilians_Middle_Shields_'.. optional_civilian_base_middle_shields, false)
        end
        if (optional_civilian_base_middle > 2 and optional_civilian_base_middle < 7) then
            ScenarioUtils.CreateArmyGroup('ARMY_17', 'Optional_Civilians_Middle_Defences_'.. optional_civilian_base_middle - 1, false);
        end
    end
    ...
    if(optional_civilian_base_side > 1) then
        ...
        if(optional_civilian_base_side_shields > 1 and optional_civilian_base_side_shields < 4) then
            ScenarioUtils.CreateArmyGroup('ARMY_17', 'Optional_Civilians_Side_Shields_'.. optional_civilian_base_side_shields, false)
        end        
        if (optional_civilian_base_side > 2 and optional_civilian_base_side < 6) then
            ScenarioUtils.CreateArmyGroup('ARMY_17', 'Optional_Civilians_Side_Defences_'.. optional_civilian_base_side - 1, false);
        end
    end
            ...
            for marker,markerpointer in pairs(ScenarioUtils.GetMarkers()) do
                if DoitIfInList(marker, DuplicateListMex, MassString, true) then
                    ...
                    CreatePropHPR('/env/common/props/hydrocarbonDeposit01_prop.bp', markerpointer.position[1],
                                  GetSurfaceHeight(markerpointer.position[1], markerpointer.position[3]),
