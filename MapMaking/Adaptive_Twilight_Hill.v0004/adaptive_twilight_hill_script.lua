...
local Tables = import('/maps/Adaptive_Twilight_Hill.v0004/Adaptive_Twilight_Hill_tables.lua')
...
local islandmex = ScenarioInfo.Options.islandmex or 2
...
local lightning = ScenarioInfo.Options.Lightning or 3
...
function Lightingstrike()
    ...
    local crystals = {}
    for i, prop in GetReclaimablesInRect(Map_Area) do
        if prop:GetBlueprint().BlueprintId == "/env/crystalline/props/rocks/cryscrystal01_prop.bp" then
            table.insert(crystals, prop)
        end
    end
    table.shuffle(crystals)
    ...
    local lightningCooldown = 1.5/lightning
    local lightningProbability = 150/(lightning + 1) + 35
    
    while table.getn(crystals) > 0 do
        WaitSeconds(lightningCooldown)
        for crystalIndex, crystal in crystals do
            if crystal:BeenDestroyed() then
                table.remove(crystals, crystalIndex)
            else
                if Random(1,100) > lightningProbability and not crystal.LightingBag then
                    crystal.LightingBag = TrashBag()
                    for _, effect in OthuyAmbientEmanation do
                        local emit = CreateAttachedEmitter(crystal,'CrysCrystal01', crystal:GetArmy(), effect)
                        crystal.LightingBag:Add( emit )
                    end
                    ForkThread(RemoveLighting, crystal)
                    WaitSeconds(lightningCooldown*0.1)
                end
            end
        end
    end
...
function RemoveLighting(self)
    WaitSeconds(1.5*lightning - 2)
    self.LightingBag:Destroy()
    self.LightingBag = nil
end
...
------------------------------------------------------------------------
    ...
    if(optional_civilian_base > 1) then
        if optional_civilian_base > 4 then
            ScenarioUtils.CreateArmyGroup('ARMY_17', 'Civ_Base_'.. (optional_civilian_base - 3), false)
            ScenarioUtils.CreateArmyGroup('ARMY_17', 'Civ_Shields', false)
        else
            ScenarioUtils.CreateArmyGroup('ARMY_17', 'Civ_Base_'.. optional_civilian_base, false)
        end
        ScenarioUtils.CreateArmyGroup('ARMY_17', 'Civ_Walls', false)
    end
