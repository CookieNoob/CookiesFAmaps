...
local Tables = import('/maps/adaptive_proteus_large.v0001/adaptive_proteus_tables.lua')
...
local plateaumex = ScenarioInfo.Options.plateaumex or 2
local underwatermex = ScenarioInfo.Options.underwatermex or 2
...
local optional_civilian_shields = ScenarioInfo.Options.optional_civilian_shields or 1
...
-- Script by CookieNoob and KeyBlue
    ...
    LOG("ULTIMATE", automex, hydroplus, optional_reclaim_middle, optional_reclaim_adaptive, middlemex, plateaumex, underwatermex, optional_reclaim, mirrormex, optional_civilian_base)
    -- get map markers
...
------------------------------------------------------------------------
    ...
    ScenarioUtils.CreateArmyGroup('ARMY_17', 'Wreckage_0', true)
    if optional_reclaim > 1 then
        ScenarioUtils.CreateArmyGroup('ARMY_17', 'Wreckage_' .. optional_reclaim, true)
    end

    if optional_civilian_base > 1 then
        ScenarioUtils.CreateArmyGroup('ARMY_17', 'Civilian_Base', false)
        ScenarioUtils.CreateArmyGroup('ARMY_17', 'Civilian_Walls', false)
        if optional_civilian_base > 2 then
            ScenarioUtils.CreateArmyGroup('ARMY_17', 'Civilian_Defences_'..optional_civilian_base, false)
        end
        if optional_civilian_shields > 1 then
            ScenarioUtils.CreateArmyGroup('ARMY_17', 'Shields_'..optional_civilian_shields, false)
        end
    end
...
-- corresponds to player one, the second to player 2 and so on... load it from the _tables.lua file
    ...
    local spwnAdditionalplateaumex = Tables.plateaumex or {}
...
-- loop over all markers of mexes in the table of the row of the missing army
            ...
            for l = 1, plateaumex - 1  do
                doit=FalseIfInList(name, spwnAdditionalplateaumex[l], MassString, doit);
            end
