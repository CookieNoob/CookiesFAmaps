...
local Tables = import('/maps/Adaptive_Metir.v0002/Adaptive_Metir_tables.lua')
...
local underwatermex = ScenarioInfo.Options.underwatermex or 3
...
------------------------------------------------------------------------
    ...
    if optional_reclaim > 1 then
        for recl = 2, optional_reclaim do
            ScenarioUtils.CreateArmyGroup('ARMY_17', 'Wrecks'..recl, true)
        end
    end
